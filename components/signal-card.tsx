import { Clock, MessageSquare, MoreVertical, ChevronRight } from "lucide-react";
...
export function SignalCard({
  ...
  return (
    <div
      className={cn(
        "rounded-2xl border p-4 sm:p-5 flex flex-col sm:flex-row items-start gap-4 bg-white relative transition-all hover:shadow-md",
        themeStyles[theme],
      )}
    >
        ...
        <div className="flex-1 min-w-0 flex flex-col gap-2">
          ...
          <div className="flex items-center flex-wrap text-xs sm:text-sm text-gray-500 gap-y-1 gap-x-2">
            {tags.map((tag, i) => (
              <span key={i} className="flex items-center">
                {tag}
                {i < tags.length - 1 && (
                  <span className="ml-2 w-1 h-1 rounded-full bg-gray-300"></span>
                )}
              </span>
            ))}
          </div>
          ...
          {(noResponseText || waitingOnText) && (
            <div className="flex items-center flex-wrap text-xs sm:text-sm text-gray-600 gap-y-1 gap-x-2 mt-1">
              ...
              {noResponseText && waitingOnText && (
                <span className="w-1 h-1 hidden sm:inline-block rounded-full bg-gray-300"></span>
              )}
      ...
      <div className="flex flex-row sm:flex-col items-center sm:items-end justify-between w-full sm:w-auto gap-4 sm:ml-4 pt-2 sm:pt-0 border-t border-gray-100/50 sm:border-0">
        ...
        <Button
          variant="outline"
          className="text-blue-600 border-blue-100 mt-0 sm:mt-2 h-8 sm:h-9 px-3 sm:px-4 text-xs sm:text-sm rounded-full font-medium hover:bg-blue-50 shrink-0"
        >
          View thread
